...
// 장기판 생성자 및 상차림 설정 포함
// 장기판 생성자
function JanggiStage(objName, stage_id, han_eatitems, cho_eatitems) {
  this.objName = objName;
  this.stageId = stage_id;
  this.stage = document.getElementById(stage_id);
  this.width = parseInt(this.stage.style.width.split('px')[0]);
  this.height = parseInt(this.stage.style.height.split('px')[0]);
...
// 위치 하이라이트 제거
JanggiStage.prototype.removePosition = function () {
  for (let div of this.pos_divs) {
    if (div.parentNode === this.stage) {
...
// 말 선택 시 이동 가능 위치 표시
JanggiStage.prototype.showPos = function (junit_obj) {

  console.log("showPos 진입", junit_obj);

   if (this.is_final === true || this.tern !== junit_obj.team || (this.is_single && junit_obj.is_computer)) {
     console.log("조건에 막혀서 return 됨",
       "is_final:", this.is_final,
       "tern:", this.tern,
       "junit_obj.team:", junit_obj.team,
       "is_single:", this.is_single,
       "junit_obj.is_computer:", junit_obj.is_computer);
     return;
  }
  ...
  const poses = junit_obj.getMoveablePos(this.getStageStatus());
  console.log("getMoveablePos 호출됨");

    console.log("가능한 위치:", poses);
  for (let i = 0; i < poses.length; i++) {
   ...
   div.onclick = function () {
     stageInstance.ChangeUnit(junit_obj.axis_x, junit_obj.axis_y, poses[i].x, poses[i].y);
   };
...
// 기본 장기말 배치
JanggiStage.prototype.placeDefaultUnits = function () {
  const defaultUnits = [
    [0, 0, Janggi.Cha], [1, 0, Janggi.Sang], [2, 0, Janggi.Ma], [3, 0, Janggi.Sa],
    [5, 0, Janggi.Sa], [6, 0, Janggi.Sang], [7, 0, Janggi.Ma], [8, 0, Janggi.Cha],
    [4, 1, Janggi.Gung], [1, 2, Janggi.Po], [7, 2, Janggi.Po],
    [0, 3, Janggi.Jole], [2, 3, Janggi.Jole], [4, 3, Janggi.Jole],
    [6, 3, Janggi.Jole], [8, 3, Janggi.Jole],
    [0, 6, Janggi.Jole], [2, 6, Janggi.Jole], [4, 6, Janggi.Jole],
    [6, 6, Janggi.Jole], [8, 6, Janggi.Jole],
    [1, 7, Janggi.Po], [7, 7, Janggi.Po],
    [4, 8, Janggi.Gung],
    [0, 9, Janggi.Cha], [3, 9, Janggi.Sa], [5, 9, Janggi.Sa], [8, 9, Janggi.Cha],
  ];

for (let [x, y, type] of defaultUnits) {
  const unit = new Units(new JanggiUnit(this.x_term, this.y_term, x, y, type), true);
  this.units.push(unit);
  this.stage.appendChild(unit.junit_obj.obj);

  console.log(" 생성된 말:", unit.junit_obj);
  console.log(" 연결된 DOM 객체:", unit.junit_obj.obj);

      const i = this.units.length - 1;
      unit.junit_obj.obj.onclick = () => {
        this.showPos(this.units[i].junit_obj);
        console.log("onclick 연결 확인", this.units[i].junit_obj.obj);

        console.log("말 등록:", unit.junit_obj.text, unit.junit_obj);
...
// 상차림 선택 후 배치
JanggiStage.prototype.init = function (setupType) {
  ...
  this.placeDefaultUnits();

  const layout = {
    "마상마상": [[1, 9, Janggi.Ma], [2, 9, Janggi.Sang], [6, 9, Janggi.Ma], [7, 9, Janggi.Sang]],
    "상마상마": [[1, 9, Janggi.Sang], [2, 9, Janggi.Ma], [6, 9, Janggi.Sang], [7, 9, Janggi.Ma]],
    "상마마상": [[1, 9, Janggi.Sang], [2, 9, Janggi.Ma], [6, 9, Janggi.Ma], [7, 9, Janggi.Sang]],
    "마상상마": [[1, 9, Janggi.Ma], [2, 9, Janggi.Sang], [6, 9, Janggi.Sang], [7, 9, Janggi.Ma]],
  };

  const extraUnits = layout[setupType || "마상마상"];
  for (let [x, y, type] of extraUnits) {
    const unit = new Units(new JanggiUnit(this.x_term, this.y_term, x, y, type), true);
    this.units.push(unit);
    this.stage.appendChild(unit.junit_obj.obj);
    const i = this.units.length - 1;
  ...
  console.log(`${setupType} 상차림 배치 완료`);
};


	JanggiStage.prototype.showComBeforePos = function(f_x, f_y, t_x, t_y) {
	...
	JanggiStage.prototype.showPos = function(junit_obj) { // 선택 장기알이 갈 수 있는 위치 보여주는 함수
		this.com_beforepos.style.display = 'none';
		this.com_nowpos.style.display = 'none';
	...
	JanggiStage.prototype.removePosition = function() { // 선택 장기알이 갈 수 있는 위치 보여주고있는것 삭제
	...
	JanggiStage.prototype.ChangeUnit = function(from_x, from_y, to_x, to_y, motion_after) {
...
var JanggiUnit = function(x_term, y_term, axis_x, axis_y, janggi_kind) {
    ...
    const pieceSize = 110; // 말의 가로/세로 크기

    // 텍스트 설정 (innerText 또는 innerHTML 중 하나만 쓰면 됩니다)
    this.obj.innerText = this.text;

    // 위치 조정
    const dx = -25;  // 미세 위치 보정
    const dy = -30;
	...
	console.log("getMoveablePos 호출됨");
    console.log("this.axis_x =", this.axis_x, "this.axis_y =", this.axis_y);
    console.log("stage_state[this.axis_x][this.axis_y] =", stage_state[this.axis_x][this.axis_y]);
    console.log("stage_state[this.axis_x][this.axis_y].junit_obj === this =", stage_state[this.axis_x][this.axis_y]?.junit_obj === this);

		var now_x = this.axis_x;
		var now_y = this.axis_y;
		for(var col = 0 ; col <= 8 ; col++) {
			for (var row = 0 ; row <= 9 ; row++) {
			  if (stage_state[col][row] && stage_state[col][row].junit_obj === this) {
                  now_x = col;
                  now_y = row;
                  col = 9; // 루프 조기 종료
                  break;
